...
ax1 = Axis(
    ...
    yscale=log10,
    yticks=(10.0 .^ collect(-2:0), [latexstring("10^{$(i)}") for i in -2:0]),
...
lines!(
    ...
    linewidth=2,
    color="red",
    label="ScatteringOptics.jl (Minor Axis)",
)
lines!(
    ax1,
    sodf[!, :uvd1] / 1e9,
    ...
    linewidth=2,
    color="blue",
    label="ScatteringOptics.jl (Major Axis)",
)
...
lines!(
    ax2,
    sodf[!, :uvd1] / 1e9,
    ferr(sodf[!, :vismin1], ehdf[!, :vismin1]);
    linewidth=2,
    color="red",
    label="Minor Axis",
)
lines!(
    ax2,
    sodf[!, :uvd1] / 1e9,
    ferr(sodf[!, :vismaj1], ehdf[!, :vismaj1]);
    linewidth=2,
    color="blue",
    label="Major Axis",
)
...
ax3 = Axis(
    ...
    xminorgridvisible=true,
    yscale=log10,
    yticks=(10.0 .^ collect(-12:0), [latexstring("10^{$(i)}") for i in -12:0]),
    yminorticks=IntervalsBetween(9),
    yminorticksvisible=true,
    yminorgridvisible=true,
)
...
lines!(
    ax4,
    sodf[!, :uvd2] / 1e6,
    ferr(sodf[!, :vismin2], ehdf[!, :vismin2]);
    linewidth=2,
    color="red",
    label="Minor Axis",
)
lines!(
    ax4,
    sodf[!, :uvd2] / 1e6,
    ferr(sodf[!, :vismaj2], ehdf[!, :vismaj2]);
    linewidth=2,
    color="blue",
    label="Major Axis",
)
