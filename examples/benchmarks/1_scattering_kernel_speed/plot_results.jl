...
for i=1:3
    ...
    ax = Axis(
        ...
        xscale = log10,
        yscale = log10,
        xticks = (10. .^ collect(1:6), [latexstring("10^{$(i)}") for i in 1:6]),
        xminorticks = IntervalsBetween(9),
        xminorticksvisible = true,
        xminorgridvisible = true,
        yticks = (10. .^ collect(-6:-1), [latexstring("10^{$(i)}") for i in -6:-1]),
    ...
    lines!(ax, sodf[!, :samples], sodf[!, dfcols[i]], linewidth = 2, color = "purple", label = "ScatteringOptics.jl")
    scatter!(ax, sodf[!, :samples], sodf[!, dfcols[i]], color = "purple")

    lines!(ax, ehdf[!, :samples], ehdf[!, dfcols[i]], linewidth = 2, color = "orange", label = "eht-imaging (Python)")
