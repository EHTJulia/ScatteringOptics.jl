...
ax = Axis(
    ...
    xscale=log10,
    yscale=log10,
    xticks=(10.0 .^ collect(3:7), [latexstring("10^{$(i)}") for i in 3:7]),
    xminorticks=IntervalsBetween(9),
    xminorticksvisible=true,
    xminorgridvisible=true,
    yticks=(10.0 .^ collect(-4:2), [latexstring("10^{$(i)}") for i in -4:2]),
...
lines!(
    ...
    linewidth=2,
    color="purple",
    label="Full Scattering",
)
lines!(
    ax,
    sodf[!, :nx] .^ 2,
    sodf[!, :ensembleaverage];
    linewidth=2,
    linestyle=:dash,
    color="purple",
    label="Diffractive Scattering Only",
)
lines!(ax, ehdf[!, :nx] .^ 2, ehdf[!, :average]; linewidth=2, color="orange")
lines!(
    ax,
    ehdf[!, :nx] .^ 2,
    ehdf[!, :ensembleaverage];
    linewidth=2,
    linestyle=:dash,
    color="orange",
)
...
scatter!(
    ax, sodf[!, :nx] .^ 2, sodf[!, :average]; color="purple", label="ScatteringOptics.jl"
)
scatter!(
    ax, ehdf[!, :nx] .^ 2, ehdf[!, :average]; color="orange", label="eht-imaging (Python)"
)
