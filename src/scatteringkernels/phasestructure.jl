...
struct PhaseStructure{T<:Number, F<:Function} <: AbstractScatteringKernel
    ...
    ϕ0::Number
    P_ϕ::F
    function PhaseStructure(α=1.38, inscale=800e5, θmaj=1.380, θmin=.703, ϕpt=81.9, λ0=1., M=2.82/5.53) 
        ...
        Bmaj = B_prefac*quadgk(ϕ -> abs(cos(ϕ0-ϕ))^α*P_ϕ(ϕ), 0, 2*π)[1]
        Bmin = B_prefac*quadgk(ϕ -> abs(sin(ϕ0-ϕ))^α*P_ϕ(ϕ), 0, 2*π)[1]
        

        Amaj = (inscale*(1.0 + M) * θmaj_rad/(FWHM * (λ0/(2.0*π)) * 2.0*π ))^2
        Amin = (inscale*(1.0 + M) * θmin_rad/(FWHM * (λ0/(2.0*π)) * 2.0*π ))^2

        return new{typeof(α), Function}(α, inscale, θmaj, θmin, ϕpt, λ0, M, ζ, A, k, Bmaj, Bmin, Qbar, C, Amaj, Amin, ϕ0, P_ϕ) 
    end
end
