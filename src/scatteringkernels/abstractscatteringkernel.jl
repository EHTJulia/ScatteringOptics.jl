...
"""
@inline function calc_dmaj(ps::AbstractScatteringKernel, r::Number)
    Bmaj = ps.Bmaj
    C = ps.C
    α = ps.α
    ζ = ps.ζ
...
"""
@inline function calc_dmin(ps::AbstractScatteringKernel, r::Number)
    Bmin = ps.Bmin
    C = ps.C
    α = ps.α
    ζ = ps.ζ
...
"""
function phase_structure_point(ps::AbstractScatteringKernel, x::Number, y::Number)
...
"""
function visibility_point(ps::AbstractScatteringKernel, u::Number, v::Number)
    b = (u, v).* (λ/(1+ps.M))
